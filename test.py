...
def start_menu_loop():
    ...
    text = font_start.render(f"Snake Mania", True, "#16A34A")
...
def randxy_st(toint=False):
    ...
    ceneters = ((cx, cy),)
    while pos in apples or pos in ceneters:
        pos = f"{random.randint(0, MSIZE[0] - 1)},{random.randint(0, MSIZE[1] - 1)}"
    if toint:
...
def init_player_on_server():
    global sock, MSIZE, teleports, apples, apple_add, name, color
    if not online:
    ...
    print("init", data_lst)
    _code, name, color, xy, _msize, _teleports, _apples, _players = data_lst[:7 + 1]
    if _msize == "0":
        init_map()
        map_data = "map;" + ",".join(map(str, MSIZE)) + ";" + "|".join(teleports) + ";"
